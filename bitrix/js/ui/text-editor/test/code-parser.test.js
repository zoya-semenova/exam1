import { mergeTokens, parse } from '../src/plugins/code/parser';

describe('Code Parser', () => {
	it('should parse a simple text', () => {
		const tokens = parse('One One\nTwo\n\nThree\n\tFour Four Four Four', false);
	...
	it('should parse a JavaScript code', () => {
		const tokens = parse('var my_var = "a"; // [comment]\nif (a === true)\n{\tlet s = 123;\n\ts++;\n}\n', false);
	...
	it('should parse simple line', () => {
		...
		tokens = parse('// comment', false);
		assert.equal(tokens.length, 1);
		assert.equal(tokens[0].type, 'comment');
		assert.equal(tokens[0].content, '// comment');

		// tokens = parse('[ini]', false);
		// tokens = parse('[', false);
		// tokens = parse(']', false);
		// tokens = parse('{', false);
		// tokens = parse('}', false);
		// tokens = parse('{brace}', false);
		// tokens = parse('{brace};', false);
		// tokens = parse(';', false);
		// tokens = parse(':', false);
		// tokens = parse('/* \n\n word */', false);
